...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: OpenVLAConfig) -> None:
        ...
        if "llama" in self.config.llm_backbone_id:
            self.vocab_size = self.config.text_config.vocab_size - self.config.pad_to_multiple_of
            self.tokenizer_len=self.vocab_size
        elif "qwen25" in self.config.llm_backbone_id and "extra" in self.config.llm_backbone_id:
            self.tokenizer_len =self.config.text_config.tokenizer_len
        # TODO: Add support for Qwen2
    ...
    def predict_action(
        ...
        discretized_actions = self.tokenizer_len - predicted_action_token_ids
