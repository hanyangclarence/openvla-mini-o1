...
def convert_openvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    if "llama" in hf_config.llm_backbone_id:
    ...
    elif "qwen25" in hf_config.llm_backbone_id and "extra" in hf_config.llm_backbone_id:
        llm_config = AutoConfig.from_pretrained("Qwen/Qwen2.5-0.5B", token=cfg.hf_token)
        hf_config.text_config=llm_config
        tokenizer.add_tokens([f"<|extra_{i}|>" for i in range(256)])
        hf_config.pad_token_id=tokenizer.pad_token_id
        hf_config.text_config.tokenizer_len=len(tokenizer)
