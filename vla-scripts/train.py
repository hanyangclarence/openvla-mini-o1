...
class TrainConfig:
    ...
    # World Size and Batch Parameters
    expected_world_size: int = 8                                    # Default world size (number of GPUs)
    global_batch_size: int = 256                                    # Total batch size across all GPUs
    per_device_batch_size: int = 32                                 # Batch size per GPU

    def __post_init__(self) -> None:
        """Lift optimization parameters from `self.vla` for ease of use =>> validate on `expected_world_size`"""
        self.vla.expected_world_size = self.expected_world_size
        self.vla.global_batch_size = self.global_batch_size
        self.vla.per_device_batch_size = self.per_device_batch_size
